...
std::vector<double> Dailyreturn(Matrice YahooData) {
    std::vector<double> DoubleCloture;
    std::vector<double> Cloture;
    DoubleCloture.push_back(YahooData.valeurs[0][4]);
    for (int i = 1; i < YahooData.n; i++) {
        DoubleCloture.push_back(YahooData.valeurs[i][4]);
        Cloture.push_back(DoubleCloture[i] / DoubleCloture[i - 1] - 1);
    }
    return Cloture;
}

double Moyenne(std::vector<double> Return) {
    double somme = 0.0;
    for (int i = 0; i < Return.size(); i++) {
        somme = somme + Return[i];
    }
    return (somme / Return.size());
}

double EcartType(std::vector<double> Return) {
    double somme = 0.0;
    double Xbarre = Moyenne(Return);
    for (int i = 0; i < Return.size(); i++) {
        somme = somme + pow(Return[i], 2) - pow(Xbarre, 2);
    }
    return sqrt(somme / Return.size());
}



std::string GetDateAsof(bool DD) {
    ...
    if (DD)
    {
        sa << (1900 + ltm->tm_year - 1);
    }
    else 
        sa << (1900 + ltm->tm_year);

    std::string Annee = sa.str();
    std::string Mois;
    std::string Jour;
    if (ltm->tm_mon < 10)
    {
