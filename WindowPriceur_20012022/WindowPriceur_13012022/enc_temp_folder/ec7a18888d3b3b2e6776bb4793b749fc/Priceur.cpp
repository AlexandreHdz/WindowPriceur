...
LRESULT CALLBACK DlgGetData(HWND hWndYahoo, UINT Msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
    {
        ...
        case IDC_VALID:
        {
            ...
            string Ligne = "";
            Ligne = "      Date          |       Openning       |       High         |          Low         |         Closing       |        Clos Aj       |        Vol         |";
            SendMessage(GetDlgItem(hWndDlgBox, IDD_OUTPUT), LB_ADDSTRING, 0, (LPARAM)Ligne.c_str());
            Ligne = "_______________________________________________________________________________________________________________";
            SendMessage(GetDlgItem(hWndDlgBox, IDD_OUTPUT), LB_ADDSTRING, 0, (LPARAM)Ligne.c_str());
            Ligne = "";
            int i = Yahoo.n-1;
            while (i>0)
            {
                Ligne = YahooDate[i] + "  |  ";
                for (int j = 1; j<7; j++) {
                    Ligne = Ligne + std::to_string(Yahoo.valeurs[i][j]) + "  |  ";
                }
                const char* charLigne = Ligne.c_str();
                SendMessage(GetDlgItem(hWndDlgBox, IDD_OUTPUT), LB_ADDSTRING, 0, (LPARAM)charLigne);
                i = i - 1;
            }
...
LRESULT CALLBACK DlgProcCallEu(HWND hWnDlg, UINT Msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
    {
        ...
        case IDC_PRICER:
        {
            ...
            /*YahooFinanceAPI api;
            Matrice Yahoo = api.get_ticker_data("spy", "2021-01-17", "2022-01-17");
            std::vector<double> Return = Dailyreturn(Yahoo);
            double Rdt = Moyenne(Return);
            double vol = EcartType(Return);*/
